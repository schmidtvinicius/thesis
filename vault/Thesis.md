...
- 
- If we instead do all operations on the dataframe by first creating a view and processing it with SQL queries, spark seems to correctly process each event once. In addition, if we process everything with built-in functions and use `saveAsTable()` to overwrite the table, the results also seem to be correctly processed. This behavior seems to happen in both Delta and Iceberg.
