...
### Datasets
In terms of datasets, it seems that there is a balance between the use of both synthetic and real datasets. In our work we want to be possible to use both types of datasets, so that we can support as many benchmarks as possible. Further, something that stood out was that most (74%) benchmarks reviewed by the authors use Apache Kafka for data ingestion. Alternatives to this include either using local files or using some type of generator for ingesting data. We will look into using Kafka for our experiments, as some papers claim that Kafka can cause bottlenecks in experiments [[Benchmarking Distributed Stream Data Processing Systems|(Karimov et al., 2018)]].

### System under test (SUT)
When it comes to systems under test (SUT), there are three systems that are frequently included in benchmarking papers, namely Apache Flink, Apache Storm and Spark Streaming. Ideally, our benchmark should be able to work with as many systems as possible, but maybe we could focus on those three first, since they seem to be the most popular ones.

### Use cases
* IoT application
* Manufacturing
* Advertisement analytic
* E-commerce
* Toll system
* Cloud application
* Social media
...
### Metrics
Finally, in terms of metrics under analysis, throughput and latency are the most common ones, with CPU-, memory- and network usage as other popular metrics.
...
* [ESPBench: The Enterprise Stream Processing Benchmark](zotero://select/library/collections/NNCK7D2G/items/Z95ZQCC6)
* [Theodolite: Scalability Benchmarking of Distributed Stream Processing Engines in Microservice Architectures](zotero://select/library/collections/NNCK7D2G/items/XI33VJGU)
* [Benchmarking Distributed Stream Data Processing Systems](zotero://select/library/collections/NNCK7D2G/items/CCEYL3D4)
